...
app domains dbConfig tableParam scalaPkg pursPkg = launchAff_ $ processResult do
  let checkString s =
  ...
  let config = { domains, table, poolConfig, scalaPkg, pursPkg }
...
dbModules :: Config -> ExceptT String Aff (Maybe Module)
dbModules config = do
  pool <- liftEffect $ newPool config.poolConfig
  if config.domains
    then Just <$> Database.domainModule pool config.scalaPkg config.pursPkg
    else for config.table $ Database.tableModule pool config.scalaPkg config.pursPkg
...
writeModule :: Module -> ExceptT String Aff Unit
