...
oneModule mod = vsep 1 Boxes.left
  let
    env =
      ...
      , allModules: [ invalidate mod ]
      }
    Tuple body imports = runCodegen env (traverse typeDecl mod.types)
    allImports = imports <> (mod.imports <#> _.exports.pursPkg >>> importModule)
  in
...
importModule :: String -> PsImport
importModule mod =
  let
    alias = maybe mod _.last $ Array.unsnoc $ String.split (Pattern ".") mod
  in
    { mod, typ: Nothing, alias: Just alias }
...
internalRef mod typ = do
  ...
  let
    fullModName = do
      modName <- mod
      let prefix = (modPrefix allModules modName) <|> defaultPrefix
      pure $ maybe modName (_ <> "." <> modName) prefix
