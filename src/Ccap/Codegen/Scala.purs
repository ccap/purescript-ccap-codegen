...
oneModule defaultPackage all mod@(Module name decls annots impts) = do
...
refData all typ modName = do
  Module _ decls annots _ <- Array.find (\(Module n _ _ _) -> n == modName) all
  ...
  case ty of
    Ref _ { mod, typ } -> do
      ...
      let external = mod >>= (\m -> Array.find (\(Module n _ _ _) -> n == m) allModules)
          Module _ ds _ _ = fromMaybe currentModule external
