...
type AnnotParam = { name :: String, value :: Maybe String }

type Annot = { name :: String, params :: Array AnnotParam }
  ...
  describe "Annotations on type declarations" do
    describe "when pretty printed" do
      it "keeps inline type annotations" $
        checkPrint tmplFile $ typeShouldHaveAnnotation "Inline" maxLen5
      it "keeps next line type annotations" $
        checkPrint tmplFile $ typeShouldHaveAnnotation "NextLine" maxLen5
      it "keeps inline record prop annotations" $
        checkPrint tmplFile $ fieldShouldHaveAnnotation "Record" "inline" maxLen5
      it "keeps next line record prop annotations" $
        checkPrint tmplFile $ fieldShouldHaveAnnotation "Record" "nextLine" maxLen5
...
typeShouldHaveAnnotation_ typeName annot =
  checkTypes $ typeShouldHaveAnnotation typeName annot

fieldShouldHaveAnnotation_ :: String -> String -> Annot -> Aff Unit
fieldShouldHaveAnnotation_ typeName propName annot =
  checkTypes $ fieldShouldHaveAnnotation typeName propName annot

checkPrint :: FilePath -> (Array TypeDecl -> Aff Unit) -> Aff Unit
checkPrint filePath check = runOrFail do
  source <- exceptAffT $ sourceTmpl filePath
  let printed = scrubEolSpaces $ prettyPrint (invalidate source.contents)
